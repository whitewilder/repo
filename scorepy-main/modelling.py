...
def score_df(scored_financial_dev, train_data, y='Class'):
    ...
    cols=['cohort_date_sas','model_crr','final_crr']

    if all(col in train_data.columns for col in cols):
        ...
        scored_financial_dev=scored_financial_dev
...
def build_model(feature_vars, train_data, test_data, oot_data, train_df, test_df, oot_df, target_col='intodef_gid'):
    ...
    scored_train_df = score_df(scored_train, train_df, y=target_col)
    scored_test_df = score_df(scored_test, test_df, y=target_col)
    scored_oot_df = score_df(scored_oot, oot_df, y=target_col)
    scored_calib_df = score_df(scored_calibration, combined_calib_df, y=target_col)
