...
def corr_drop(df, target, threshold=0.9, how='max_corr', order_list=None):
    ...
    if how == 'max_corr':
        ...
        def process_pair(col1, col2, corr_value):
            """
            Helper function to handle the decision of which variable to drop.
            """
            # Ignore pairs where one of the variables is already marked for dropping
        ...
        # Process each correlated pair recursively
        for col1, col2, corr_value in correlated_pairs:
            process_pair(col1, col2, corr_value)    
    else:
