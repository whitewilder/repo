...
def df_AR_AUC(df, Score="Score", target="target", ascending=False, method="GRADE", k=10):
    ...
    # Calculate k_bar
    k_bar = (
        0.5 * (aux02['fraction_defs'].shift(-1) - aux02['fraction_defs']) * 
        (aux02['fraction_obligors'].shift(-1) - aux02['fraction_obligors'])
    )[0:-1].sum() + \
    (aux02['fraction_defs'] * 
     (aux02['fraction_obligors'].shift(-1) - aux02['fraction_obligors']))[0:-1].sum() + \
         0.5 * aux02['fraction_defs'].iloc[0] * aux02['fraction_obligors'].iloc[0] -0.5
    
    # Calculate lambda_bar
    lambda_bar = total_defs / total_count
    
    # Calculate AR
    AR = k_bar / (0.5 * (1 - lambda_bar))

    # Calculate AUROC
    AUROC = (AR + 1) / 2
    
    # Calculate GINI
    GINI = AR * (1 - lambda_bar)
...
def calculate_auc(df, feature, target="intodef"):
    """
    ...
    training_auc = df_AR_AUC(data, Score=feature, target=target, ascending=ascending, method="CONCORDENT")[1]["AUROC"]  # Assuming the method returns AUC in the first index
    ...
    if training_auc < 0.50:  # Check if the AUC is less than the baseline
        training_auc = df_AR_AUC(data, Score=feature, target=target, ascending=not ascending , method="CONCORDENT")[1]["AUROC"]
